        ...
        private void Generate()
        {
            ...
            for (int row = 0; row < size.y; row++)
            {
                for (int column = 0; column < size.x; column++)
                {
                    ...
                    Add<Position>(pieceEntity).Value = new Vector2IntFixed(column, row, SimConstants.GridSubdivison);
                    Add<Velocity>(pieceEntity).Value = new Vector2IntFixed(0, 0, SimConstants.GridSubdivison);
